...
- `session.header: SessionHeader` — detached, deep-frozen creation metadata (`version`, `id`, `createdAt`, optional `cwd`/`parentSession`/`seedLength`/`delegationDepth`/`agentPreset`/`systemPrompt`). A non-empty `systemPrompt` stores the session-specific model persona that resume and fork must reinstall. Construction validates the durable record and requires its id to match `session.id`.
