...
- Push each authored branch report into the main Session as injected context. `[[silence]]` remains internal and never appears in the public transcript. A main reply that incorrectly returns the marker for an ordinary user message receives one direct-answer retry in the same Session; a second marker fails the public reply without rendering it.
