...
function recognitionPersona(shadow) {
  ...
  return [
    ...
    withWorker
      ...
      : '需要 → {"relevant": true, "brief": "直接给出最终「闪念」:一句第一人称、口语化的话(20~50 字),用「我查了一下/我看了下」这类如实说法开头,不要用「我想起来了/我记得」"}',
    '除调用工具外不要输出任何其他内容。',
  ].join('\n')
}
...
function workerPersona(shadow) {
  return [
    ...
    '产出一句简短、第一人称、口语化的「闪念」(20~50 字),用「我查了一下/我看了下」这类如实说法开头(刚查了什么就说什么),不要用「我想起来了/我记得」,把最关键的一两个信息自然带出来。',
